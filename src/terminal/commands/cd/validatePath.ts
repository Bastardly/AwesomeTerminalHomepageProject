...
function noHashOrQueriesInMiddleElements(
    char: string,
    pathElementsWithoutLast: string[],
) {
...
export default function validatepath(path: string): string {
    ...
    const errors: string[] = [
        ...
        noHashOrQueriesInMiddleElements("#", pathElements)
            ? "You can only use a hash on the last item."
            : "",
        noHashOrQueriesInMiddleElements("?", pathElements)
            ? "You can only use a query on the last item."
            : "",
        tooManyDots(path) ? "Seems like you've written a dot too many ..." : "",
    ];

    return errors
        .reduce((errorMsg, error) => {
            if (!error.length) {
                return errorMsg;
            }
